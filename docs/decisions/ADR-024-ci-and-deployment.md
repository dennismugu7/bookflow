...
**Note the estimate above was optimistic.** It put iOS at 20 billable minutes per run from a 1m44s
build. The build has grown with the app — roughly 8 minutes now — so the real figure is nearer 80,
and the conclusion that followed from 20 no longer follows from 80.
