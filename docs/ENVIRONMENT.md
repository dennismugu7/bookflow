...
| **GitHub Actions minutes** | every CI run, every deploy, the e2e gate | ADR-024 | **EXHAUSTED — 2026-08-16.** GitHub Free: **2,000 minutes a month** for private repositories. Usage reached **$12.09 gross, fully covered by the included allowance — nothing was charged.** The Actions budget is **$0 with stop-usage on**, a deliberate decision and not an oversight: GitHub is not load-bearing for the running product the way Supabase is. So runs are **refused rather than billed**. **Resets around 2026-08-31.** | `github.com/settings/billing` |
| **What a refusal looks like** | recognising this state next time | ADR-024 | A job reports **`conclusion: failure` with zero steps, no logs, no runner name, and a two-second duration** — it never started. Run `31936920827` is the specimen: `verify`, `contracts` and `mobile` all failed that way on a commit whose only change was Markdown. **It does not look like a broken build**, which is exactly the trap — the instinct is to debug the code. | `gh api repos/dennismugu7/bookflow/actions/runs/31936920827/jobs` |
| **Consequence: no PR can merge until the reset** | — | `DEFINITION_OF_DONE.md` | The checklist requires **CI green end to end**, and a green run cannot be produced while runs are refused. **That gate is not weakened, waived or worked around to get a merge** — an unproducible green is a reason to wait, never a reason to lower the bar. The queue is in `docs/BUILD_LOG.md` §8. | — |
