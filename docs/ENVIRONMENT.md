...
| **The "Confirm signup" email template on `bookflow-staging`** | the entry flow verifies by CODE, and the template decides whether a code is ever sent | ADR-027 | **OWED — a change nobody can make from this repository.** The app now verifies sign-up through GoTrue's OTP path (`verifyOTP type: signup`), which needs the six-digit token in the message body. **Supabase's default Confirm-signup template contains only `{{ .ConfirmationURL }}`**, so as it stands a new owner receives a link and no code, types nothing into the verification sheet, and cannot finish. **Do this: Supabase dashboard → `bookflow-staging` → Authentication → Email Templates → Confirm signup → include `{{ .Token }}` in the body.** Hosted configuration, not `supabase/config.toml`, so it is invisible to CI and to a fresh clone — which is why it is a row here rather than a comment in the client. | send a signup confirmation to a reachable address and read it: the message must contain a six-digit code, not only a link |
| **The "Reset password" email template on `bookflow-staging`** | password reset verifies by CODE, and the template decides whether a code is ever sent | ADR-027 | **OWED — the same fix as the Confirm-signup row above, on a second template.** The reset flow exchanges a six-digit token for a recovery session (`verifyOTP type: recovery`), and Supabase's default Reset-password template contains only `{{ .ConfirmationURL }}`. Without the token an owner who has forgotten their password gets a link, types nothing into "Please verify your identity", and stays locked out — **which is worse than the sign-up case, because the person hitting it has no other way in.** **Do this: Supabase dashboard → `bookflow-staging` → Authentication → Email Templates → Reset password → include `{{ .Token }}` in the body.** Hosted configuration, not `supabase/config.toml`. | request a reset for a reachable address and read the mail: it must contain a six-digit code, not only a link |
| **The `public-media` Storage bucket on `bookflow-staging`** | ADR-011's public, content-addressed bucket — where banners, team photos and portfolio images are stored | ADR-011 | **exists — verified 2026-08-22.** An unauthenticated probe for an object nobody uploaded answers `NoSuchKey`, not `NoSuchBucket`, which is the bucket present AND public; 12 consecutive probes agreed. **Local is now provisioned from the repository** — `supabase/config.toml` declares `[storage.buckets.public-media]`, so a fresh clone and CI get the shape staging has. **Staging is still hand-made in the dashboard**, and that half is what the `smoke-staging` job's storage section now re-checks on every deploy. `POST /v1/me/business/images` writes to a bucket named `public-media` through Storage's HTTP API with the service-role key, and returns the object's public URL for the row that references it. **Do this: Supabase dashboard → `bookflow-staging` → Storage → New bucket → name `public-media`, PUBLIC bucket (public read) enabled.** Public read is the whole point and is ADR-011's decision: these are brand assets on a booking page anyone may see, and the private bucket that needs a signed URL is the payment-proof one — **see the `private-media` row below; this API does touch it now, and that sentence used to say it did not.** Key layout is `{businessId}/{purpose}/{uuid}.{ext}`, so nothing is guessable from a business name and no object is ever overwritten. **Until the bucket exists, uploads answer 503 `storage-unavailable`** — the endpoint fails honestly rather than recording a URL that serves nothing. | `curl -s -o /dev/null -w '%{http_code}' "https://vvborjxraxdeflrllqwh.supabase.co/storage/v1/object/public/public-media/probe"` → `400`/`404` once the bucket exists, `404` with a `Bucket not found` body while it does not |
| **The `private-media` Storage bucket on `bookflow-staging`** | ADR-011's private bucket — where client payment proofs are stored, reachable only through a signed URL | ADR-011 | **not public — verified 2026-08-22; existence still UNCONFIRMED from here.** An unauthenticated probe answers `NoSuchBucket` (12 of 12), which rules out the bucket being public but does **not** distinguish private-and-present from absent — those are the same answer, as the verification column has always said. Only a request carrying the anon key separates them, and this machine does not hold the staging key. **The `smoke-staging` job does**, and its storage section now asserts exactly this pair on every deploy to main: existence with the anon key, visibility without one. **Close this row when that job has passed once.** Local is provisioned from the repository — `supabase/config.toml` declares `[storage.buckets.private-media]` with `public = false`. `POST /v1/public/salons/{handle}/bookings` writes the uploaded proof here with the service-role key and stores only the object KEY; `GET /v1/me/business/bookings/{id}/payment-proof` mints a ~5-minute signed URL for the owner. **Do this: Supabase dashboard → `bookflow-staging` → Storage → New bucket → name `private-media`, PUBLIC bucket DISABLED.** **Creating it public makes this whole change cosmetic** — the keys would resolve, the endpoint would keep working, and the objects would be world-readable again with nothing reporting it. Note the failure mode differs from the public bucket's: an image upload answers 503 when its bucket is missing, but a *booking* proceeds without its proof and only logs a warning, because an appointment is worth more than an attachment. So a missing bucket here looks like success. **Also owed, separately: the proofs already written to `public-media` under `*/proof/*` by the previous version.** They are client PII at permanently public URLs; the migration deliberately does not delete storage objects, so they must be removed by hand. | bucket exists and is private: `curl -s -o /dev/null -w '%{http_code}' "https://vvborjxraxdeflrllqwh.supabase.co/storage/v1/object/public/private-media/probe"` → **`400`/`404` with a `Bucket not found`-style body is NOT sufficient here** — that is also what a missing bucket returns. **And on the self-hosted `storage-api` image the CI stack runs, it is not even stable**: measured 2026-08-22 over 20 rounds, a private bucket answered `NoSuchKey` on that path 5 times and a bucket that did not exist answered `NoSuchKey` 3 times. Hosted Storage did not reproduce it (36/36 consistent), which is why the smoke check takes three probes and requires them to agree rather than trusting one. Confirm in the dashboard that the bucket is listed and its **Public** toggle is off, then book with a proof against staging and check `GET /v1/me/business/bookings` reports `hasPaymentProof: true` |
| **The `bookflow-staging-web` Render static site** | the client booking site — what a salon's shared link actually opens | ADR-001, ADR-024 | **OWED — `render.yaml` declares it and nothing in this repository can create it.** Render does not pick up a new service from a blueprint automatically. **Do this: Render dashboard → the Bookflow blueprint → Sync / Apply, and confirm a second service appears with `runtime: static`, `buildCommand: npm ci && npm run web:build`, `staticPublishPath: ./apps/web/dist`, and the `/*` → `/index.html` REWRITE.** The rewrite is not optional: every URL this site serves is `/:handle`, no such file exists in `dist/`, and without it every shared link and every page reload 404s — only `/` would load, which is the one URL nobody has. **Then verify the assigned hostname**: Render appends a suffix when a service name is not globally unique (the API is `bookflow-api-staging-gabm`, not `bookflow-api-staging`). If it does so here, `AppConfig.defaultWebBaseUrl` in `apps/mobile/lib/platform/config.dart` is wrong and every booking link an owner shares 404s — that constant currently assumes no suffix. Auto-deploy is `off` for both services, so a deploy is triggered from the dashboard or by the same API call CI already makes for the API. | `curl -s -o /dev/null -w '%{http_code}' https://bookflow-staging-web.onrender.com/some-unknown-handle` → **`200`** (the rewrite serving index.html; the app then renders its own "not taking bookings yet"). A `404` means the rewrite is missing or the hostname carries a suffix — `curl -sI https://bookflow-staging-web.onrender.com` distinguishes the two |
| **Security headers on the `bookflow-staging-web` static site** | the web app's only defence against sniffing, framing and injected script — it renders HTML, unlike the API | — | **OWED, AND IT IS A SEPARATE SYNC FROM CREATING THE SERVICE.** `render.yaml` now carries a `headers:` block for the static site (X-Content-Type-Options, X-Frame-Options, Referrer-Policy and a real Content-Security-Policy). **Render applies header changes only when the blueprint is re-synced** — a service that already exists keeps serving without them until then, so this is owed again after every change to that block, not just once. **Do this: Render dashboard → the Bookflow blueprint → Sync / Apply**, then re-check with the command opposite. The API's equivalents come from `@fastify/helmet` and ship with the code, so they need no dashboard step — that asymmetry is the thing to remember. Note the CSP's `connect-src` names the API origin explicitly: if the API's hostname ever changes, **the site stops being able to reach it and the browser reports only a console error**, which is a failure with no server-side signal at all. | `curl -sI https://bookflow-staging-web.onrender.com/ \| grep -i -E 'content-security-policy\|x-frame-options\|x-content-type-options\|referrer-policy'` → all four present. An empty result means the blueprint has not been synced since the headers were added |
...
| Render web service `bookflow-api-staging` | the deployed staging API | ADR-024 amendment, ADR-034 | **exists and is live — 2026-08-15.** Id **`srv-da06mqvlk1mc73f98qv0`**, region **`frankfurt`** (= `eu-central-1`, same region as the staging database), plan `free`, runtime `docker`, health check `/health`, **`autoDeployTrigger: off`**. URL **`https://bookflow-api-staging-gabm.onrender.com`**. Ids and the URL are identifiers, not secrets. **THE FREE PLAN SLEEPS, AND THAT IS A PRODUCT PROBLEM, NOT A NUISANCE.** Render's free instance type stops the service after ~15 minutes without inbound traffic. A device test on 2026-08-22 measured `GET /health` returning **HTTP 200 in 23.2 seconds** from cold. The Flutter app's Dio timeouts were 10s connect / 20s receive, so **every first request after an idle period was abandoned before the server finished waking** — and surfaced as the unclassified "Something went wrong. Check your connection and try again", because the client cannot tell a cold start from a dead network. Three compensating controls now carry it, all in the app and none of them a fix: 60s timeouts (`platform/api_client.dart`), a fire-and-forget `GET /health` at launch so the wake overlaps the welcome screen (`platform/warmup.dart`), and a "the server may be waking up" notice after 8s (`AuthSubmitButton`). **A paid Render instance does not sleep and removes the cause**; at that point the timeouts can go back to 10/20 and the warm-up becomes pointless. Note this affects OUR API only — Supabase Auth is hosted and does not sleep, so login, verification and password reset were never affected. | `curl -H "Authorization: Bearer $RENDER_API_KEY" https://api.render.com/v1/services/srv-da06mqvlk1mc73f98qv0` · cold-start timing: `curl -s -o /dev/null -w '%{time_total}\n' https://bookflow-api-staging-gabm.onrender.com/health` after ≥15 min idle |
