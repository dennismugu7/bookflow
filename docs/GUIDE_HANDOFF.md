...
**Phases 0, 1, 2 and 3 of the project manual are complete.** Phase 3 closes at PR 4c,
which is built, reviewed, approved and **waiting on a merge that cannot happen until CI
minutes reset** (see §6).
...
- **Nine CI jobs**: `verify`, `contracts`, `mobile`, `ios-build`, `migrate-staging`,
  `deploy-staging`, `smoke-staging`, `e2e-staging`. Every one proven red before green.
- **40 ADRs** in `docs/decisions/`, an append-only register. All foundation-level (F)
  triage items are closed.

**Read these to orient** (he can attach them or paste sections):

| File | What it is |
|---|---|
| `CLAUDE.md` | The governing rules, loaded into every Claude Code session. §7 carries the editing and reporting rules. |
| `DEFINITION_OF_DONE.md` | The gates. Binary by design. |
| `docs/BUILD_LOG.md` | Phase history. **§8 is the merge queue** waiting on CI minutes. |
| `docs/ENVIRONMENT.md` | World-state — what exists, what is provisioned, what is exhausted. Mutable by design. |
| `docs/analysis/05-triage.md` | Every unresolved item, classified F / S / D. |
| `docs/analysis/09-phase3-close.md` | Phase 3's close-out. §7 is the close itself. |
| `docs/decisions/ADR-040-*.md` | When a phase may close with unsatisfiable items. Read this before ever waiving a gate. |
...
- **Supabase free tier** allows two projects per organisation; one slot is taken by an
  unrelated project.
