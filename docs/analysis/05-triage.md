...
| K77 | K | **NEW (PR 4c).** Phase 0 of the next slice produces a **written list of acceptance criteria**, before design, each phrased so a test can be named after it. `DEFINITION_OF_DONE.md` requires every criterion to map to a named test, and Phase 3 could not tick that box because Phase 3's Phase 0 produced no criteria to map. **This is not a documentation chore.** Criteria written *after* the tests are derived from them, so the mapping is green by construction and proves nothing — criteria must constrain tests, never the reverse. The rule now lives in `CLAUDE.md` §7. **ADR-040 §3.1 authorises the Phase 3 miss once and says explicitly that the exception is spent**: from Phase 4 onward this item is unfinished-and-blocking, not waivable. | DECIDED — `CLAUDE.md` §7, ADR-040 | S | blocks the first Phase 0 after 2026-08-15 |
| K78 | K | **NEW (PR 4c), and largely closed in the same PR.** `--dart-define-from-file` constant-folds its values into `kernel_blob.bin`, recoverable with `grep -a` — verified on a throwaway build (`docs/analysis/10-e2e-credential-in-artefact.md` §1). **The password was inside the test APK. It no longer is:** CI performs the password grant itself and compiles in only an `access_token` with one hour of life, with a literal placeholder where a refresh token would go, so the artefact cannot renew itself. `E2E_STAGING_PASSWORD` never enters a build (§5). **What remains open is a bounded window** — between the grant and the test's sign-out, the artefact holds a live token — so an artifact uploaded from a run that failed before sign-out publishes a credential with up to an hour left. Rotation stays **trigger-based, not scheduled**: a calendar rotation of a privilege-less staging account is theatre whose skipping is invisible. **Triggers:** (1) an upload/cache/persist step is added to `e2e-staging` **and** a run has failed before its sign-out — lowered in severity by §5; (2) the account is granted a membership, a business, or any privilege beyond its own profile row; (3) staging is ever pointed at real data. **The rule that outlives the rotation policy: this account must never be given a membership** — the day it has one, none of the containment reasoning survives. | DECISION | D | nothing today; triggers 2 and 3 are the ones that matter |
| K80 | K | **NEW (business setup, PR 5a review).** **May a non-owner membership rename a business?** `renameBusinessForUser` scopes its `update` with an `exists` subquery over `memberships` that matches **any** role, while `findBusinessOwnedBy` filters `role = 'owner'`. The asymmetry is deliberate and recorded in that function's doc comment: *"which business is MINE"* has one correct meaning, but **whether a stylist at a salon may rename it is a product decision nobody has made**, and adding `role = 'owner'` would decide it as "no" by implementation rather than by decision. **Unreachable today** — `ck_memberships_role` permits only `'owner'`, so no non-owner membership can exist. **Trigger: `ck_memberships_role` widening (I9)**, which is the same event that makes it reachable and the same event `uq_memberships_one_owner_per_user` is partial in order to allow. Decide it with the role vocabulary, not after. The same question then applies to every other write this module gains. | DECISION | D | none today; #20's rename affordance once roles widen |
| K81 | K | **NEW (business setup, owner's review pass).** **Sign-out is a one-way door: tapping it locks an owner out of the app.** Screen #5 (`/setup`) and screen #17 (`/account`) both offer it, and **there is no login screen and no login route** — the signed-out shell's two buttons, `'Create for free'` and `'Sign in'`, are both `onPressed: null`. A session already in secure storage survives a relaunch, so an owner who never signs out stays in; **one who signs out cannot get back in.** Screen #5's sign-out exists because `/setup` is the only destination the redirect allows an owner with no business, so without it that owner has no exit from the app at all (criterion 61, deviation entry 15) — **the alternative to a one-way door was a room with no door.** **The decision to ship it rests on there being NO USERS**: production has no Supabase slot (ADR-023), no tag has ever been cut (ADR-024), and the only accounts that exist are two staging test accounts. **That reasoning expires at the first real signup**, which is the trigger. **BLOCKING on the login slice** — not deferrable, because the cost is a locked-out owner rather than a missing feature. | DECISION | S | #5, #17, the signed-out shell, and the login screen that does not exist |
| K82 | K | **NEW (business setup, owner's review pass).** **Every creation failure shows the same connection error, including the one that is not a connection problem.** `create_business_screen.dart` renders one string for every failed submission: *"That did not save. Check your connection and try again."* **A 409 `business-already-exists` reads as a network fault** — an owner who already has a business is told to check their connection. `grep` for `409` or `business-already-exists` across `apps/mobile/lib` finds no UI branch; the distinction exists in the API contract (ADR-014 has the client branch on `type`) and is discarded at the screen. **Reachability is thin and thin is not none:** the redirect keeps an owner who has a business away from `/setup`, so the realistic path is a **double-tap** on Next — two submissions in flight, the second answered 409 — or a stale screen after the business was created elsewhere. **BLOCKING on PR 5b**, the e2e branch, where the screen's error handling is already in the diff being changed. | DECISION | S | #5 |
