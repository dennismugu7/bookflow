...
| K78 | K | **NEW (PR 4c).** The e2e credential is **compiled into the test binary** — `--dart-define-from-file` constant-folds it into `kernel_blob.bin`, recoverable with `grep -a` (verified on a throwaway build; `docs/analysis/10-e2e-credential-in-artefact.md`). Nothing uploads artifacts from `e2e-staging`, so nothing leaks today, **but the protection is the absence of a step rather than a property of the value.** Rotation is **trigger-based, not scheduled** — a calendar rotation of a privilege-less staging account is theatre whose skipping is invisible. **Triggers:** (1) any step is added to `e2e-staging` that uploads, caches or persists anything; (2) the account is granted a membership, a business, or any privilege beyond its own profile row; (3) staging is ever pointed at real data. **The rule that matters more than the rotation: this account must never be given a membership** — the day it has one, a password inside a debug build on a hosted runner is protecting real business data. | DECISION | D | nothing today; binds the moment an upload step is added |
