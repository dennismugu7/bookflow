...
| A15 | A | **NEW (PR 2c).** The "created but unemailable" compensation path has never run against real GoTrue, and PR 4's staging smoke test must exercise it. ADR-037's amendment names the trigger: the admin API does not validate address deliverability and the public endpoints do (spike 002 S2, observed on staging), so `POST /admin/users` succeeds for an address `POST /resend` then rejects, leaving an account that can never be confirmed. **That asymmetry does not reproduce on the local stack** — probed 2026-08-14, `@bookflow.test`, `@localhost`, `@a`, `@invalid` and `@example.invalid` were all accepted by *both* the admin API and `/resend` — so `signup.integration.test.ts` drives the branch with an injected HTTP 500 from `/resend` instead. That proves the compensation code runs and both rows are removed; it does **not** prove the trigger exists, that GoTrue answers the way the spike recorded, or that the real failure is classified as anything other than `unavailable`. **PR 4 verifies it on staging with a real unemailable address**, asserting no `auth.users` row and no `user_profiles` row survive. Until then, the only compensation path exercised against real GoTrue is the profile-insert one. | **VERIFY** | S | none — blocks PR 4's staging smoke test |
| K74 | K | **NEW (PR 3a review).** What does the client do when our API answers **401**? Today: **nothing**. `apps/mobile/lib/platform/api_client.dart` sets `validateStatus: status < 500`, so a 401 returns a `Response` rather than throwing, and no interceptor, provider or screen inspects the status. The redirect only reacts to `supabase_flutter`'s own session stream, so the case that matters — GoTrue still believes the session is live while our API rejects the token (refresh revoked elsewhere, key rotation, clock skew) — leaves the user on an owner screen watching every read fail, with no path back to the signed-out shell. **Currently unreachable and therefore not yet a bug**: `apiClientProvider` is referenced by no other file, because 3a ships no screen that calls the API. **It becomes reachable in PR 3b**, which is the first PR to make a real request, and it must be answered there. The likely shape is a Dio error interceptor that signs the session out on 401 so the existing redirect does the rest — but "sign out on any 401" is wrong if a 401 can also mean "this particular resource is not yours", so the decision needs the API's problem slugs (`missing-token`, `invalid-token`, `expired-token`) rather than the bare status. | DECISION | S | none yet — blocks PR 3b, the first client code that calls the API |
